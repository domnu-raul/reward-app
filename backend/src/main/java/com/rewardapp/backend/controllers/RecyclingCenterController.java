...
public class RecyclingCenterController {
  ...
  @GetMapping("/all")
  public ResponseEntity<CollectionModel<RecyclingCenter>>
  getAll(HttpServletRequest request,
    ...
    if (latlng != null) {
      ...
      recyclingCenters =
          recyclingCenterService.getClosest(latlng.get(0), latlng.get(1), page);
    } else
  ...
  @PostMapping
  public ResponseEntity<RecyclingCenter>
  create(@RequestBody RecyclingCenter recyclingCenter,
         HttpServletRequest request) {
  ...
  @DeleteMapping("/{id}")
  public ResponseEntity<HttpEntity<Boolean>>
      delete(@PathVariable("id") Long id, HttpServletRequest request) {
    authService.validateAdminRequest(request);

    Boolean status = recyclingCenterService.deleteById(id);
    HttpEntity<Boolean> httpEntity = new HttpEntity<>(status);

    return ResponseEntity.status(status ? HttpStatus.OK : HttpStatus.CONFLICT)
        .body(httpEntity);
  }
}
